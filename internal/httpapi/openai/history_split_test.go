...
func TestBuildOpenAICurrentInputContextTranscriptUsesNumberedHistorySections(t *testing.T) {
	...
	if !strings.Contains(transcript, "# TOOL_GATEWAY_HISTORY.txt") {
...
func TestApplyCurrentInputFileUploadsFirstTurnWithNumberedHistoryTranscript(t *testing.T) {
	...
	if upload.Filename != "TOOL_GATEWAY_HISTORY.txt" {
	...
	if !strings.Contains(out.FinalPrompt, "Continue from the latest state in the attached TOOL_GATEWAY_HISTORY.txt context.") {
	...
	if !strings.Contains(out.PromptTokenText, "# TOOL_GATEWAY_HISTORY.txt") || !strings.Contains(out.PromptTokenText, "=== 1. USER ===") {
...
func TestApplyCurrentInputFilePreservesFullContextPromptForTokenCounting(t *testing.T) {
	...
	if !strings.Contains(out.PromptTokenText, "# TOOL_GATEWAY_HISTORY.txt") || !strings.Contains(out.PromptTokenText, "=== 1. SYSTEM ===") {
		t.Fatalf("expected prompt token text to include numbered history transcript, got %q", out.PromptTokenText)
	}
	if !strings.Contains(out.PromptTokenText, "Continue from the latest state in the attached TOOL_GATEWAY_HISTORY.txt context.") {
...
func TestApplyCurrentInputFileUploadsFullContextFile(t *testing.T) {
	...
	upload := ds.uploadCalls[0]
	if upload.Filename != "TOOL_GATEWAY_HISTORY.txt" {
		t.Fatalf("expected TOOL_GATEWAY_HISTORY.txt upload, got %q", upload.Filename)
	}
	if upload.ModelType != "vision" {
		t.Fatalf("expected vision model type for vision request, got %q", upload.ModelType)
	}
	uploadedText := string(upload.Data)
	for _, want := range []string{"# TOOL_GATEWAY_HISTORY.txt", "=== 1. SYSTEM ===", "=== 2. USER ===", "=== 3. ASSISTANT ===", "=== 4. TOOL ===", "=== 5. USER ===", "system instructions", "first user turn", "hidden reasoning", "tool result", "latest user turn", promptcompat.ThinkingInjectionMarker} {
		if !strings.Contains(uploadedText, want) {
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "TOOL_GATEWAY_HISTORY.txt" {
		t.Fatalf("expected first upload to be TOOL_GATEWAY_HISTORY.txt, got %q", ds.uploadCalls[0].Filename)
	}
	if ds.uploadCalls[1].Filename != "TOOL_GATEWAY_TOOLS.txt" {
		t.Fatalf("expected second upload to be TOOL_GATEWAY_TOOLS.txt, got %q", ds.uploadCalls[1].Filename)
	}
...
func TestApplyCurrentInputFileCarriesHistoryText(t *testing.T) {
	...
	if !strings.Contains(out.HistoryText, "# TOOL_GATEWAY_HISTORY.txt") || !strings.Contains(out.HistoryText, "=== 1. SYSTEM ===") {
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(historyText, "# TOOL_GATEWAY_HISTORY.txt") || !strings.Contains(historyText, "=== 1. SYSTEM ===") {
	...
	if !strings.Contains(promptText, "Continue from the latest state in the attached TOOL_GATEWAY_HISTORY.txt context.") {
...
func TestCurrentInputFileWorksAcrossAutoDeleteModes(t *testing.T) {
	for _, mode := range []string{"none", "single", "all"} {
		t.Run(mode, func(t *testing.T) {
			...
			promptText, _ := ds.completionReq["prompt"].(string)
			if !strings.Contains(promptText, "Continue from the latest state in the attached TOOL_GATEWAY_HISTORY.txt context.") || strings.Contains(promptText, "first user turn") || strings.Contains(promptText, "latest user turn") {
