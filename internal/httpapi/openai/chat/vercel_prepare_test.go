...
func TestVercelInternalSecret(t *testing.T) {
	t.Run("prefer explicit secret", func(t *testing.T) {
		...
		t.Setenv("TOOL_GATEWAY_ADMIN_KEY", "admin-fallback")
	...
	t.Run("fallback to admin key", func(t *testing.T) {
		t.Setenv("TOOL_GATEWAY_VERCEL_INTERNAL_SECRET", "")
		t.Setenv("TOOL_GATEWAY_ADMIN_KEY", "admin-fallback")
	...
	t.Run("empty when env missing", func(t *testing.T) {
		t.Setenv("TOOL_GATEWAY_VERCEL_INTERNAL_SECRET", "")
		t.Setenv("TOOL_GATEWAY_ADMIN_KEY", "")
		if got := vercelInternalSecret(); got != "" {
...
func TestStreamLeaseTTL(t *testing.T) {
	t.Setenv("TOOL_GATEWAY_VERCEL_STREAM_LEASE_TTL_SECONDS", "120")
	if got := streamLeaseTTL(); got != 120*time.Second {
		t.Fatalf("expected ttl=120s, got %v", got)
	}
	t.Setenv("TOOL_GATEWAY_VERCEL_STREAM_LEASE_TTL_SECONDS", "invalid")
...
func TestHandleVercelStreamPrepareUsesHalfwidthDSMLToolPrompt(t *testing.T) {
	...
	h := &Handler{
		Store:   mockOpenAIConfig{},
		Auth:    streamStatusAuthStub{},
		Backend: &inlineUploadBackendStub{},
	}
...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	stdReq := promptcompat.StandardRequest{
		...
		FinalPrompt:             "Continue from the latest state in the attached TOOL_GATEWAY_HISTORY.txt context. Available tool descriptions and parameter schemas are attached in TOOL_GATEWAY_TOOLS.txt; use only those tools and follow the tool-call format rules in this prompt.",
		PromptTokenText:         "# TOOL_GATEWAY_HISTORY.txt\n\n=== 1. USER ===\nhello\n\n# TOOL_GATEWAY_TOOLS.txt\nAvailable tool descriptions and parameter schemas for this request.\n\nYou have access to these tools:\n\nTool: search\nDescription: search docs\nParameters: {\"type\":\"object\"}\n",
		HistoryText:             "# TOOL_GATEWAY_HISTORY.txt\n\n=== 1. USER ===\nhello\n",
