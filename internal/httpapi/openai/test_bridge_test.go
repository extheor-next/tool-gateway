...
func (h *openAITestSurface) deps() shared.Deps {
	...
	return shared.Deps{Store: h.Store, Auth: h.Auth, Backend: h.Backend, ChatHistory: h.ChatHistory}
}

func (h *openAITestSurface) chatHandler() *chat.Handler {
	if h.chat == nil {
		deps := h.deps()
		h.chat = &chat.Handler{Store: deps.Store, Auth: deps.Auth, Backend: deps.Backend, ChatHistory: deps.ChatHistory}
	}
	return h.chat
}
...
func (h *openAITestSurface) responsesHandler() *responses.Handler {
	if h.responses == nil {
		deps := h.deps()
		h.responses = &responses.Handler{Store: deps.Store, Auth: deps.Auth, Backend: deps.Backend, ChatHistory: deps.ChatHistory}
	}
	return h.responses
}
...
func (h *openAITestSurface) filesHandler() *files.Handler {
	if h.files == nil {
		deps := h.deps()
		h.files = &files.Handler{Store: deps.Store, Auth: deps.Auth, Backend: deps.Backend, ChatHistory: deps.ChatHistory}
	}
	return h.files
}
...
func (h *openAITestSurface) embeddingsHandler() *embeddings.Handler {
	if h.embeddings == nil {
		deps := h.deps()
		h.embeddings = &embeddings.Handler{Store: deps.Store, Auth: deps.Auth, Backend: deps.Backend, ChatHistory: deps.ChatHistory}
	}
	return h.embeddings
}
