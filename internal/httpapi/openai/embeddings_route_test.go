...
func TestEmbeddingsRouteContract(t *testing.T) {
	store, resolver := newResolverWithConfigJSON(t, `{"keys":["test-token"],"embeddings":{"provider":"deterministic"}}`)
...
func TestEmbeddingsRouteProviderMissing(t *testing.T) {
	store, resolver := newResolverWithConfigJSON(t, `{"keys":["test-token"]}`)
	h := &openAITestSurface{Store: store, Auth: resolver}
	r := chi.NewRouter()
	registerOpenAITestRoutes(r, h)
