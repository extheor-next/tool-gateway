...
func TestCaptureRawSampleWritesPersistentSample(t *testing.T) {
	t.Setenv("TOOL_GATEWAY_RAW_STREAM_SAMPLE_ROOT", t.TempDir())
	...
	if got := rec.Header().Get("X-Tool-Gateway-Sample-Id"); got != "my-sample-01" {
		t.Fatalf("expected sample id header my-sample-01, got %q", got)
	}
	if got := rec.Header().Get("X-Tool-Gateway-Sample-Upstream"); got != filepath.Join(os.Getenv("TOOL_GATEWAY_RAW_STREAM_SAMPLE_ROOT"), "my-sample-01", "upstream.stream.sse") {
	...
	sampleDir := filepath.Join(os.Getenv("TOOL_GATEWAY_RAW_STREAM_SAMPLE_ROOT"), "my-sample-01")
...
func TestCaptureRawSampleCombinesContinuationCaptures(t *testing.T) {
	...
	sampleDir := filepath.Join(os.Getenv("TOOL_GATEWAY_RAW_STREAM_SAMPLE_ROOT"), "my-sample-02")
...
func TestCaptureRawSampleReturnsErrorWhenNoNewCaptureRecorded(t *testing.T) {
	root := t.TempDir()
	t.Setenv("TOOL_GATEWAY_RAW_STREAM_SAMPLE_ROOT", root)
	devcapture.Global().Clear()
	defer devcapture.Global().Clear()
