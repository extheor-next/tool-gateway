...
func (h *Handler) captureRawSample(w http.ResponseWriter, r *http.Request) {
	...
	copyHeader(w.Header(), rec.Header())
	w.Header().Set("X-Tool-Gateway-Sample-Id", saved.SampleID)
	w.Header().Set("X-Tool-Gateway-Sample-Dir", saved.Dir)
	w.Header().Set("X-Tool-Gateway-Sample-Meta", saved.MetaPath)
	w.Header().Set("X-Tool-Gateway-Sample-Upstream", saved.UpstreamPath)
	w.WriteHeader(rec.Code)
	_, _ = io.Copy(w, bytes.NewReader(rec.Body.Bytes()))
}
