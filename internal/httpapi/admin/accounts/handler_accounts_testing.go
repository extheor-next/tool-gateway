...
func (h *Handler) testAccount(ctx context.Context, acc config.Account, model, message string) map[string]any {
	...
	sessionID, err := h.Backend.CreateSession(proxyCtx, authCtx, 1)
	if err != nil {
		newToken, loginErr := h.Backend.Login(proxyCtx, acc)
		if loginErr != nil {
	...
	// 获取会话数量
	sessionStats, sessionErr := h.Backend.GetSessionCountForToken(proxyCtx, token)
	...
	pow, err := h.Backend.GetPow(proxyCtx, authCtx, 1)
	if err != nil {
	...
	resp, err := h.Backend.CallCompletion(proxyCtx, authCtx, payload, pow, 1)
	if err != nil {
...
func (h *Handler) deleteAllSessions(w http.ResponseWriter, r *http.Request) {
	...
	token, err := h.Backend.Login(proxyCtx, acc)
	if err != nil {
	...
	// 删除所有会话
	err = h.Backend.DeleteAllSessionsForToken(proxyCtx, token)
	if err != nil {
		...
		_ = h.Store.UpdateAccountToken(acc.Identifier(), token)
		authCtx.DeepSeekToken = token
		if retryErr := h.Backend.DeleteAllSessionsForToken(proxyCtx, token); retryErr != nil {
