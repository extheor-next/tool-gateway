...
func TestClaudeDirectAppliesCurrentInputFile(t *testing.T) {
	...
	if ds.uploads[0].Filename != "TOOL_GATEWAY_HISTORY.txt" {
	...
	if len(full.Messages) != 1 || !strings.Contains(full.Messages[0].Content, "Continue from the latest state in the attached TOOL_GATEWAY_HISTORY.txt context.") {
...
func TestClaudeCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploads[0].Filename != "TOOL_GATEWAY_HISTORY.txt" || ds.uploads[1].Filename != "TOOL_GATEWAY_TOOLS.txt" {
