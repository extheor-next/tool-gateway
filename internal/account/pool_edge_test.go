...
func TestPoolEmptyNoAccounts(t *testing.T) {
	t.Setenv("TOOL_GATEWAY_ACCOUNT_MAX_INFLIGHT", "2")
	t.Setenv("TOOL_GATEWAY_ACCOUNT_MAX_QUEUE", "")
...
func TestPoolMaxQueueSizeOverride(t *testing.T) {
	...
	t.Setenv("TOOL_GATEWAY_ACCOUNT_MAX_QUEUE", "5")
	t.Setenv("TOOL_GATEWAY_CONFIG_JSON", `{"keys":["k1"],"accounts":[{"email":"acc1@example.com","token":"t1"}]}`)
	pool := NewPool(config.LoadStore())
	status := pool.Status()
