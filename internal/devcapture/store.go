...
func NewFromEnv() *Store {
	...
	if raw, ok := os.LookupEnv("TOOL_GATEWAY_DEV_PACKET_CAPTURE"); ok {
		enabled = parseBool(raw)
	}
	limit := parseIntWithDefault(os.Getenv("TOOL_GATEWAY_DEV_PACKET_CAPTURE_LIMIT"), defaultLimit)
	if limit < 1 {
		limit = defaultLimit
	}
	if limit > maxLimit {
	...
	maxBodyBytes := parseIntWithDefault(os.Getenv("TOOL_GATEWAY_DEV_PACKET_CAPTURE_MAX_BODY_BYTES"), defaultMaxBodyBytes)
