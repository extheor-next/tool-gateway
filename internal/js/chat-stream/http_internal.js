...
function internalSecret() {
  return asString(process.env.TOOL_GATEWAY_VERCEL_INTERNAL_SECRET) || asString(process.env.TOOL_GATEWAY_ADMIN_KEY);
}
...
function buildInternalGoHeaders(req, opts = {}) {
  const headers = {
    ...
    'x-tool-gateway-target-account': asString(header(req, 'x-tool-gateway-target-account')),
    'x-vercel-protection-bypass': resolveProtectionBypass(req),
  };
  if (opts.withInternalToken) {
    headers['x-tool-gateway-internal-token'] = internalSecret();
  }
...
function resolveProtectionBypass(req) {
  ...
  return asString(process.env.VERCEL_AUTOMATION_BYPASS_SECRET) || asString(process.env.TOOL_GATEWAY_VERCEL_PROTECTION_BYPASS);
}
