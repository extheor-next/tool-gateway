...
func (r *Runner) startServer(ctx context.Context) error {
	...
	r.serverLogFd = logFd
	cmd := exec.CommandContext(ctx, "go", "run", "./cmd/tool-gateway")
	cmd.Stdout = logFd
	cmd.Stderr = logFd
	cmd.Env = prepareServerEnv(os.Environ(), map[string]string{
		"PORT":                          strconv.Itoa(port),
		"TOOL_GATEWAY_CONFIG_PATH":      r.configCopyPath,
		"TOOL_GATEWAY_AUTO_BUILD_WEBUI": "false",
		"TOOL_GATEWAY_CONFIG_JSON":      "",
	})
