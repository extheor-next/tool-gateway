...
func TestCORSPreflightAllowsThirdPartyRequestedHeaders(t *testing.T) {
	...
	req.Header.Set("Access-Control-Request-Headers", "authorization, x-stainless-os, x-stainless-runtime, x-tool-gateway-internal-token")
	...
	if strings.Contains(allowHeaders, "x-tool-gateway-internal-token") {
...
func TestBuildCORSAllowHeadersKeepsDefaultsWithoutRequest(t *testing.T) {
	...
	for _, want := range []string{"content-type", "x-goog-api-key", "anthropic-version", "x-tool-gateway-source"} {
