...
- Docker / Vercel：从 `config.json` 生成 Base64，填入 `TOOL_GATEWAY_CONFIG_JSON`，也可以直接填原始 JSON

```bash
TOOL_GATEWAY_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
```

Vercel 一键部署可先只填 `TOOL_GATEWAY_ADMIN_KEY`，部署后在 `/admin` 导入配置，再通过 “Vercel 同步” 写回环境变量。
...
OpenAI `/v1/*` 仍是规范路径。对于只配置 Tool Gateway 根地址的客户端，同一套 OpenAI handler 也通过根路径快捷路由暴露：`/models`、`/models/{id}`、`/chat/completions`、`/responses`、`/responses/{response_id}`、`/embeddings`、`/files`、`/files/{file_id}`。

服务器端记录本质上是 上游适配器响应归档：OpenAI Chat、OpenAI Responses、Claude Messages、Gemini GenerateContent 等直连上游适配器 的生成接口，在收到上游响应后会于各协议回译/裁剪前写入记录；列表按请求创建时间倒序展示，流式请求会在生成过程中持续刷新状态与详情。WebUI「API 测试」发出的请求也会进入该记录。
...
> 说明：`/v1/models` 返回的是规范化后的 上游模型 ID；常见 alias 仅用于请求入参解析，不会在该接口中单独展开返回。带 `-nothinking` 后缀的模型表示无论请求里是否显式开启 thinking / reasoning，都会强制关闭思考输出。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | ✅ | 支持 上游模型 + 常见 alias（如 `gpt-5.5`、`gpt-5.4-mini`、`gpt-5.3-codex`、`o3`、`claude-opus-4-6`、`claude-sonnet-4-6`、`gemini-2.5-pro`、`gemini-3.1-pro`、`gemini-3-flash` 等）；若模型名带 `-nothinking` 后缀，则强制关闭 thinking / reasoning |
...
- token 计数优先透传上游 upstream SSE（如 `accumulated_token_usage` / `token_usage`）；仅在上游缺失时回退本地估算。失败/中断型结束（例如 `response.failed`）可能不会携带 `usage`
...
无需鉴权。入参支持 alias（例如 `gpt-4o`），返回的是映射后的 上游模型对象。
...
需要业务鉴权。查询 上游上传文件的当前状态，并返回 OpenAI `file` 对象；未找到匹配文件时返回 `404`。
...
- 不会输出 `signature_delta`（当前上游未提供可验证签名）
...
请求体兼容 Gemini `contents` / `tools` 字段，模型名可用 alias 自动映射到 上游模型；若路径中的模型名带 `-nothinking` 后缀，则最终会映射到对应的无思考模型。
...
- token 计数优先透传上游 upstream SSE（如 `accumulated_token_usage` / `token_usage`）；仅在上游缺失时回退本地估算
