...
flowchart LR
    ...
    subgraph ToolGateway["Tool Gateway 4.x (Modular HTTP Surface + PromptCompat Core)"]
    ...
    Completion -.upstream runtime.-> ProviderRuntime
    Auth --> UpstreamClient
...
- **Backend**: Go (`cmd/tool-gateway/`, `api/`, `internal/`), no Python runtime
...
Tool Gateway treats model names as a routing layer from client-facing IDs to upstream model IDs:

- Clients can use common names such as `gpt-*`, `claude-*`, `gemini-*`, or your own business-specific aliases.
- `model_aliases` in `config.example.json` controls the final upstream model mapping.
- `/v1/models` returns the models exposed by the gateway; individual upstream adapters may add capabilities such as thinking, search, or vision.
- OpenAI, Claude, and Gemini entrypoints share the same alias and Tool Calling semantics, so clients can switch protocols without rewriting tool definitions.

### Claude Code integration pitfalls (validated)

- Set `ANTHROPIC_BASE_URL` to the Tool Gateway root URL (for example `http://127.0.0.1:5001`). Claude Code sends requests to `/v1/messages?beta=true`.
...
- If your environment has proxy variables, set `NO_PROXY=127.0.0.1,localhost,<your_host_ip>` for Tool Gateway to avoid proxy interception of local traffic.
- If tool calls are rendered as plain text and not executed, first verify the model output uses the recommended halfwidth-pipe DSML block: `<|DSML|tool_calls><|DSML|invoke name="..."><|DSML|parameter name="...">...`. Tool Gateway also accepts legacy canonical XML: `<tool_calls><invoke name="..."><parameter name="...">...`; legacy `<tools>` / `<tool_call>` / `<tool_name>` / `<param>`, `<function_call>`, `tool_use`, or standalone JSON `tool_calls` are not executed and stay plain text.
...
- Docker / Vercel: generate Base64 from `config.json` and inject as `TOOL_GATEWAY_CONFIG_JSON`, or paste raw JSON directly
...
The default `docker-compose.yml` uses `ghcr.io/cjackhwang/ds2api:latest` and maps host port `6011` to container port `5001`. If you want `5001` exposed directly, set `TOOL_GATEWAY_HOST_PORT=5001` (or adjust the `ports` mapping).
...
2. After deployment, open `/admin` and login with `TOOL_GATEWAY_ADMIN_KEY` shown in Zeabur env/template instructions.
3. Import / edit config in Admin UI (it will be written and persisted to `/data/config.json`).

Fresh Zeabur volumes can start without `/data/config.json`; Tool Gateway will boot with an empty file-backed config and create the file on the first Admin UI save.

For manual deployment without the template, create a Zeabur GitHub service, keep Root Directory as `/`, build with the repo-root `Dockerfile`, mount a persistent volume at `/data`, set `PORT=5001`, `TOOL_GATEWAY_ADMIN_KEY=your-strong-secret`, and `TOOL_GATEWAY_CONFIG_PATH=/data/config.json`, then expose HTTP port `5001`. See [docs/DEPLOY.en.md](docs/DEPLOY.en.md#manual-deployment-without-the-template) for the full guide.
...
3. Set environment variables (minimum: `TOOL_GATEWAY_ADMIN_KEY`; recommended to also set `TOOL_GATEWAY_CONFIG_JSON`)
4. Deploy
...
Recommended: convert `config.json` to Base64 locally, then paste into `TOOL_GATEWAY_CONFIG_JSON` to avoid JSON formatting mistakes:
...
# 2. Configure
cp config.example.json config.json
# Edit config.json with your upstream account/credential info and client API keys
...
> **WebUI auto-build**: On first local startup, if the WebUI static directory is missing, Tool Gateway auto-runs `npm ci --prefix webui` (only when dependencies are missing) and `npm run build --prefix webui -- --outDir static/admin --emptyOutDir` (requires Node.js; `TOOL_GATEWAY_STATIC_ADMIN_DIR` can override the static directory). You can also build manually: `./scripts/build-webui.sh`
...
- `accounts`: managed upstream accounts or credentials, with proxy/name/remark metadata where supported.
...
For business endpoints (`/v1/*`, `/anthropic/*`, Gemini routes), Tool Gateway supports two modes:

| Mode | Description |
| --- | --- |
| **Managed account** | Use a key from `config.keys` via `Authorization: Bearer ...` or `x-api-key`; Tool Gateway auto-selects an account |
| **Reject unauthorized key** | If the token is not in `config.keys`, the request returns `401 invalid api key` |

Optional header `X-Tool-Gateway-Target-Account`: Pin a specific managed account (value is email or mobile).
...
```
Per-account inflight = TOOL_GATEWAY_ACCOUNT_MAX_INFLIGHT (default 2)
Recommended concurrency = account_count × per_account_inflight
Queue limit = TOOL_GATEWAY_ACCOUNT_MAX_QUEUE (default = recommended concurrency)
...
When `tools` is present in the request, Tool Gateway performs anti-leak handling:
...
This is for debugging issues such as Responses reasoning streaming and tool-call handoff. When enabled, Tool Gateway stores the latest N upstream conversation payload pairs (request body + upstream response body), defaulting to 20 entries with auto-eviction; each response body is capped at 5 MB by default.
...
- `request_body`: full payload sent to the upstream adapter
...
This project is provided for learning, research, personal experimentation, and internal validation only. No commercial authorization is granted, and no warranty of stability, fitness, or results is provided.
The author and repository maintainers are not responsible for any direct or indirect loss, account suspension, data loss, legal risk, or third-party claims arising from use, modification, distribution, deployment, or reliance on this project.
