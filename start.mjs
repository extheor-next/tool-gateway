...
const CONFIG = {
  ...
  adminKey: process.env.TOOL_GATEWAY_ADMIN_KEY || 'admin',
  webuiDir: join(__dirname, 'webui'),
  staticAdminDir: process.env.TOOL_GATEWAY_STATIC_ADMIN_DIR || join(__dirname, 'static', 'admin'),
};
...
async function buildBackend() {
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn('go', ['build', '-o', BINARY, './cmd/tool-gateway'], {
      cwd: __dirname,
      stdio: 'inherit',
      shell: isWindows,
...
async function startBackendDev() {
  ...
  const proc = spawn('go', ['run', './cmd/tool-gateway'], {
    cwd: __dirname,
    stdio: 'inherit',
    shell: isWindows,
    env: { ...process.env,
      ...
      TOOL_GATEWAY_ADMIN_KEY: CONFIG.adminKey,
...
async function showMenu() {
  ...
  console.log(`  TOOL_GATEWAY_ADMIN_KEY:  ${colors.cyan}${CONFIG.adminKey}${colors.reset}`);
  ...
  console.log(`${colors.dim}  自定义: TOOL_GATEWAY_ADMIN_KEY=密钥 PORT=5001 node start.mjs${colors.reset}`);
...
${colors.cyan}常用环境变量:${colors.reset}
  ...
  TOOL_GATEWAY_ADMIN_KEY   管理员密钥 (默认: admin)
  TOOL_GATEWAY_CONFIG_PATH 配置文件路径 (默认: config.json)
  GOPROXY            Go 模块代理 (默认: https://goproxy.cn,direct)
  NPM_REGISTRY       npm 镜像源 (默认: https://registry.npmmirror.com)

${colors.cyan}示例:${colors.reset}
  TOOL_GATEWAY_ADMIN_KEY=mykey PORT=8080 node start.mjs dev
