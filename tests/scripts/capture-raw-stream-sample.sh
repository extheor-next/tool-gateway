...
API_KEY="${5:-}"
ADMIN_KEY="${TOOL_GATEWAY_ADMIN_KEY:-admin}"
...
cleanup() {
  ...
  pkill -f "cmd/tool-gateway" >/dev/null 2>&1 || true
}
trap cleanup EXIT

TOOL_GATEWAY_CONFIG_PATH="$CONFIG_PATH" \
...
for line in headers:
  if line.lower().startswith('x-tool-gateway-sample-dir:'):
...
headers=pathlib.Path(sys.argv[1]).read_text().splitlines()
for line in headers:
  if line.lower().startswith('x-tool-gateway-sample-id:'):
    print(line.split(':',1)[1].strip())
    raise SystemExit
print('')
PY
