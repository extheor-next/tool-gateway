...
- When upstream returns a thinking-only response with no visible text, the Go main path and the Vercel Node streaming path retry once in the same upstream session: it appends the prompt suffix `"Previous reply had no visible output. Please regenerate the visible final answer or tool call now."` and sets `parent_message_id`. If that same-account retry would still end as `429 upstream_empty_output`, managed-account mode switches to the next available account, creates a fresh session, and retries the original payload once before returning 429.
- Citation/reference marker boundary: streaming output hides upstream `[citation:N]` / `[reference:N]` placeholders by default; non-stream output converts upstream search reference markers into Markdown links.
...
- Docker / Vercel: generate Base64 from `config.json`, then set `TOOL_GATEWAY_CONFIG_JSON`, or paste raw JSON directly

```bash
TOOL_GATEWAY_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
```

For Vercel one-click bootstrap, you can set only `TOOL_GATEWAY_ADMIN_KEY` first, then import config at `/admin` and sync env vars from the "Vercel Sync" page.
...
OpenAI `/v1/*` paths are canonical. For clients configured with the bare Tool Gateway service URL, the same OpenAI handlers are also exposed through root shortcuts: `/models`, `/models/{id}`, `/chat/completions`, `/responses`, `/responses/{response_id}`, `/embeddings`, `/files`, and `/files/{file_id}`.
...
> Note: `/v1/models` returns normalized upstream model IDs. Common aliases are accepted only as request input and are not expanded as separate items in this endpoint.

### Model Alias Resolution

For `chat` / `responses` / `embeddings`, Tool Gateway follows a wide-input/strict-output policy:

1. Match upstream model IDs first.
...
Aliases with a `-nothinking` suffix also map to the corresponding forced no-thinking upstream model.
...
| --- | --- | --- | --- |
| `model` | string | ✅ | upstream models + common aliases (`gpt-5.5`, `gpt-5.4-mini`, `gpt-5.3-codex`, `o3`, `claude-opus-4-6`, `gemini-2.5-pro`, `gemini-3.1-pro`, `gemini-3-flash`, etc.); `-nothinking` suffixes force thinking / reasoning off |
...
- Token counting prefers pass-through from upstream upstream SSE (`accumulated_token_usage` / `token_usage`), and only falls back to local estimation when upstream usage is absent. Failed/interrupted endings (for example `response.failed`) may not include `usage`
...
No auth required. Alias values are accepted as path params (for example `gpt-4o`), and the returned object is the mapped upstream model.
...
If `tool_choice=required` is violated in stream mode, Tool Gateway emits `response.failed` then `[DONE]` (no `response.completed`).
...
Business auth required. Retrieves the current upstream upload status for a file and returns an OpenAI `file` object. Returns `404` when no matching file is found.

---

## Claude-Compatible API

Besides `/anthropic/v1/*`, Tool Gateway also supports shortcut paths: `/v1/messages`, `/messages`, `/v1/messages/count_tokens`, `/messages/count_tokens`.
...
- `signature_delta` is not emitted (the active upstream does not provide verifiable thinking signatures)
...
Request body accepts Gemini-style `contents` / `tools`. Model names can use aliases and are mapped to upstream models.
...
- Token counting prefers pass-through from upstream upstream SSE (`accumulated_token_usage` / `token_usage`), and only falls back to local estimation when upstream usage is absent
...
- `X-Tool-Gateway-Sample-Id`
- `X-Tool-Gateway-Sample-Dir`
- `X-Tool-Gateway-Sample-Meta`
- `X-Tool-Gateway-Sample-Upstream`
...
```json
{
  ...
  "release_url": "https://github.com/extheor-next/tool-gateway/releases/tag/v3.0.0",
