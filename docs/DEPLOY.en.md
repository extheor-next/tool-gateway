...
- **Environment variable**: `TOOL_GATEWAY_CONFIG_JSON` (recommended for Vercel; supports raw JSON or Base64)
...
- Docker / Vercel: generate `TOOL_GATEWAY_CONFIG_JSON` (Base64) from `config.json` and inject it
...
#   TOOL_GATEWAY_ADMIN_KEY=your-admin-key
# Optionally set the host port:
#   TOOL_GATEWAY_HOST_PORT=6011
...
- **Port**: Tool Gateway listens on `5001` by default; the template sets `PORT=5001`.
- **Persistent config**: the template mounts `/data` and sets `TOOL_GATEWAY_CONFIG_PATH=/data/config.json`. On a fresh volume, Tool Gateway starts with an empty file-backed config; after importing config in Admin UI, it will be written and persisted to this path.
- **`open /app/config.json: permission denied`**: this means the instance is trying to persist runtime tokens to a read-only path (commonly `/app` inside the image).  
  ...
  1. Set a writable path explicitly: `TOOL_GATEWAY_CONFIG_PATH=/data/config.json` (and mount a persistent volume at `/data`);
  2. If you bootstrap with `TOOL_GATEWAY_CONFIG_JSON` and do not need runtime writeback, keep env-backed mode (`TOOL_GATEWAY_ENV_WRITEBACK` disabled);
...
- **First login**: after deployment, open `/admin` and login with `TOOL_GATEWAY_ADMIN_KEY` shown in Zeabur env/template instructions (recommended: rotate to a strong secret after first login).
...
| `TOOL_GATEWAY_ADMIN_KEY` | Strong random string | Required admin login key. |
| `TOOL_GATEWAY_CONFIG_PATH` | `/data/config.json` | Recommended persistent config path. |
| `LOG_LEVEL` | `INFO` | Optional log level. |
| `TOOL_GATEWAY_CONFIG_JSON` | Raw JSON or Base64 JSON | Optional config bootstrap from env. |
| `TOOL_GATEWAY_ENV_WRITEBACK` | `1` | Optional; enable only when using `TOOL_GATEWAY_CONFIG_JSON` and you want the initial config written to `/data/config.json`. |

7. Expose HTTP port `5001`. The health check path can be `/healthz`.
8. After deployment, open `/admin`, login with `TOOL_GATEWAY_ADMIN_KEY`, then import or edit config in Admin UI. A fresh volume does not need `/data/config.json` up front; the service boots first and creates the file on the first save.
...
- **`open /app/config.json: permission denied`**: the config path still points at the read-only image directory; mount `/data` and set `TOOL_GATEWAY_CONFIG_PATH=/data/config.json`.
- **Config disappears after restart**: check that the `/data` persistent volume is mounted on this service. If you use `TOOL_GATEWAY_CONFIG_JSON` but want Admin UI saves persisted, enable `TOOL_GATEWAY_ENV_WRITEBACK=1`.
...
| --- | --- |
| `TOOL_GATEWAY_ADMIN_KEY` | Admin key (required) |
| `TOOL_GATEWAY_CONFIG_JSON` | Config content, raw JSON or Base64 (optional, recommended) |

4. **Deploy**

### 3.1.1 Recommended Input (avoid `TOOL_GATEWAY_CONFIG_JSON` mistakes)

If you prefer faster one-click bootstrap, you can leave `TOOL_GATEWAY_CONFIG_JSON` empty first, then open `/admin` after deployment, import config, and sync it back to Vercel env vars from the "Vercel Sync" page.
...
```bash
# Run in repo root
TOOL_GATEWAY_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
echo "$TOOL_GATEWAY_CONFIG_JSON"
```

If you choose to preconfigure before first deploy, set these vars in Vercel Project Settings -> Environment Variables:

```text
TOOL_GATEWAY_ADMIN_KEY=replace-with-a-strong-secret
TOOL_GATEWAY_CONFIG_JSON=<the single-line Base64 output above>
```
...
| --- | --- | --- |
| `TOOL_GATEWAY_ACCOUNT_MAX_INFLIGHT` | Per-account inflight limit | `2` |
| `TOOL_GATEWAY_ACCOUNT_MAX_QUEUE` | Waiting queue limit | `recommended_concurrency` |
| `TOOL_GATEWAY_GLOBAL_MAX_INFLIGHT` | Global inflight limit | `recommended_concurrency` |
| `TOOL_GATEWAY_ENV_WRITEBACK` | When `TOOL_GATEWAY_CONFIG_JSON` is present, auto-write to `TOOL_GATEWAY_CONFIG_PATH` and switch to file-backed mode after success (`1/true/yes/on`) | Disabled |
| `TOOL_GATEWAY_VERCEL_INTERNAL_SECRET` | Hybrid streaming internal auth | Falls back to `TOOL_GATEWAY_ADMIN_KEY` |
| `TOOL_GATEWAY_VERCEL_STREAM_LEASE_TTL_SECONDS` | Stream lease TTL | `900` |
| `TOOL_GATEWAY_RAW_STREAM_SAMPLE_ROOT` | Raw stream sample root for saving/reading samples | `tests/raw_stream_samples` |
| `TOOL_GATEWAY_STATIC_ADMIN_DIR` | WebUI static asset directory | `static/admin` |
| `TOOL_GATEWAY_AUTO_BUILD_WEBUI` | Whether local startup auto-builds missing WebUI assets (`1/true/yes/on` or `0/false/no/off`) | Enabled outside Vercel |
| `VERCEL_TOKEN` | Vercel sync token | — |
| `VERCEL_PROJECT_ID` | Vercel project ID | — |
| `VERCEL_TEAM_ID` | Vercel team ID | — |
| `TOOL_GATEWAY_CHAT_HISTORY_PATH` | Chat history storage path (must be set to `/tmp/chat_history.json` on Vercel, otherwise unavailable due to read-only filesystem) | `data/chat_history.json` |
| `TOOL_GATEWAY_VERCEL_PROTECTION_BYPASS` | Deployment protection bypass for internal Node→Go calls | — |
...
4. Node connects directly to the upstream backend, relays SSE in real-time to client (including OpenAI chunk framing and tools anti-leak sieve)
...
- **Option C**: Set `VERCEL_AUTOMATION_BYPASS_SECRET` (or `TOOL_GATEWAY_VERCEL_PROTECTION_BYPASS`) for internal Node→Go calls
...
```text
TOOL_GATEWAY_CHAT_HISTORY_PATH=/tmp/chat_history.json
```
...
#   - accounts: upstream accounts (email or mobile + password)
...
On first local startup, if the WebUI static directory is missing, Tool Gateway automatically attempts to build it (requires Node.js/npm; when dependencies are missing it runs `npm ci --prefix webui`, then `npm run build --prefix webui -- --outDir <static-dir> --emptyOutDir`). The default static directory is `static/admin/`, and `TOOL_GATEWAY_STATIC_ADMIN_DIR` can override it.
...
```bash
# Disable auto-build
TOOL_GATEWAY_AUTO_BUILD_WEBUI=false go run ./cmd/tool-gateway

# Force enable auto-build
TOOL_GATEWAY_AUTO_BUILD_WEBUI=true go run ./cmd/tool-gateway
```
