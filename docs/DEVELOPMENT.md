...
| 上游适配器调用、登录、PoW、代理 | `internal/deepseek/client`、`internal/deepseek/transport` |
...
- 输出归档：`internal/responsehistory` 在协议回译/裁剪前记录 上游适配器 assistant text / thinking；即使工具调用已被对外响应转成结构化 `tool_calls` 并从可见正文剔除，后台历史仍应保留原始 DSML / XML 片段，方便排查格式漂移。
