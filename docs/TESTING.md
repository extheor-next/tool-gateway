...
如果你只想跳过这些 preflight 检查，可以直接运行 `go run ./cmd/tool-gateway-tests --no-preflight`。
...
| `--admin-key` | Admin 密钥 | `TOOL_GATEWAY_ADMIN_KEY` 环境变量 |
...
| 位置 | 格式 |
| --- | --- |
| 请求头 | `X-Tool-Gateway-Test-Trace: <trace_id>` |
...
当遇到 upstream provider 工具调用解析问题时，可以使用以下方法：
...
```bash
go run ./cmd/tool-gateway-tests --no-preflight
```
...
- 更完整的协议级行为结构说明见 [upstream providerSSE行为结构说明-2026-04-05.md](./upstream providerSSE行为结构说明-2026-04-05.md)。
