# Tool Gateway Architecture & Project Layout
...
│   ├── tool-gateway/                    # Main service bootstrap
│   └── tool-gateway-tests/              # E2E testsuite CLI bootstrap
...
│   ├── completionruntime/                # Shared completion startup, collection, empty-output/account-switch retry
│   ├── config/                           # Config loading/validation/hot reload
│   ├── deepseek/                         # Upstream client/protocol/transport
│   │   ├── client/                       # Login/session/completion/upload/delete calls
│   │   ├── protocol/                     # Upstream URLs, constants, skip path/pattern
...
│   ├── promptcompat/                     # API request -> upstream web-chat plain-text compatibility
│   ├── rawsample/                        # Raw sample read/write and management
│   ├── responsehistory/                  # upstream provider response archive and session snapshots
...
flowchart LR
    ...
    subgraph RUNTIME[Shared runtime]
        ...
        Backend[internal/deepseek/client]
...
- `internal/promptcompat`: compatibility core for turning OpenAI/Claude/Gemini requests into upstream web-chat plain-text context.
- `internal/assistantturn`: Go output-side canonical semantics, converting upstream SSE collection results and stream finalization state into assistant turns and centralizing thinking, tool call, citation, usage, stop/error behavior.
- `internal/completionruntime`: shared Go completion execution helpers for upstream session/PoW/call startup, non-stream collection, empty-output retry, and one managed-account fresh retry before a final 429; streaming paths use it to start upstream requests, continue to use `internal/stream` for real-time consumption, and use `assistantturn` during finalization.
...
- `internal/responsehistory`: upstream provider response archive, saving assistant text, thinking, raw tool-call fragments, and streaming detail before protocol rendering/trimming.
...
- `internal/claudeconv`: Claude API request to upstream provider format conversion.
...
- On first local startup, if `static/admin` is missing, Tool Gateway may auto-build it (Node.js required).
...
- Deep topics: `docs/toolcall-semantics.md`, `docs/upstream providerSSE行为结构说明-2026-04-05.md`
