...
- `current_input_file`：全局生效的上下文拆分策略；默认开启，阈值 `min_chars` 为 `0`。DeepSeek / OpenAI 兼容供应商会将完整上下文上传为 `TOOL_GATEWAY_HISTORY.txt` 文件；Claude 供应商（无 Files API）则截断保留最近 `max_keep_messages`（默认 40）条消息。
