...
export default function DashboardShell({ token, onLogout, config, fetchConfig, showMessage, message, onForceLogout, isVercel }) {
    ...
    const renderTab = () => {
        switch (activeTab) {
            ...
            case 'settings':
                return <SettingsContainer config={config} onRefresh={fetchConfig} onMessage={showMessage} authFetch={authFetch} onForceLogout={onForceLogout} isVercel={isVercel} />
            default:
                return null
        }
    }
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="bg-background rounded-lg p-3 border border-border shadow-sm">
                                <div className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider mb-0.5 opacity-70">{t('sidebar.providers')}</div>
                                <div className="text-lg font-bold text-foreground leading-tight">{config.external_ai?.base_url ? 1 : 0}</div>
                            </div>
                            <div className="bg-background rounded-lg p-3 border border-border shadow-sm">
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                <header className="lg:hidden h-14 flex items-center justify-between px-4 border-b border-border bg-card">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-sm">Tool Gateway</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <LanguageToggle />
                        <button
                ...
                <div className="flex-1 overflow-auto bg-background p-4 lg:p-10">
                    <div className="max-w-6xl mx-auto space-y-4 lg:space-y-6">
                        ...
                        <div className="animate-in fade-in duration-500">
                            <Suspense fallback={<TabLoadingFallback label={activeNavItem?.label || 'Tool Gateway'} />}>
                                {renderTab()}
                            </Suspense>
                        </div>
                    </div>
