...
const DEFAULT_FORM = {
    ...
    current_input_file: { enabled: true, min_chars: 0, max_keep_messages: 40 },
...
function fromServerForm(data) {
    ...
    return {
        ...
        current_input_file: {
            ...
            max_keep_messages: Number(data.current_input_file?.max_keep_messages || 40),
        },
        thinking_injection: {
...
function toServerPayload(form) {
    ...
    return {
        ...
        current_input_file: {
            ...
            max_keep_messages: Number(form.current_input_file?.max_keep_messages || 40),
        },
        thinking_injection: {
...
export function useSettingsForm({ apiFetch, t, onMessage, onRefresh, onForceLogout, isVercel = false }) {
    ...
    const downloadExportFile = useCallback(async () => {
        ...
        const filename = `tool-gateway-config-backup-${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}-${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}.json`
